...
str(RUZAFA)
attach(RUZAFA)
# Los datos se han recgido desde 2020-09-17 hasta 2022-04-10
max(dateObserved)
min(dateObserved)
...
#¿Cual es la calle que mas ha molestado a los vecinos en los tres ultimos años?
RUZAFA %>% group_by(entityId)%>% summarise(MaxMolestia= max(LAeq_den)) %>% filter(MaxMolestia == max(MaxMolestia))
